...
label0 = Label(root,text="Enter parameters: ",font=fontStyle,anchor=W)
...
def open():
    top=Toplevel()
    top.title("analysis")
    Label(top,text="Done").grid(row=0,column=0)
    Button(top,text="Exit",command=top.destroy).grid(row=1,column=0)
...
#Buttons
Button(root,text="Submit",command=open).grid(row=13,column=0,columnspan=5,sticky=W+E,pady=5)
Button(root,text="CLear All",command=clear).grid(row=14,column=0,columnspan=5,sticky=W+E)
